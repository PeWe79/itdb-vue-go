...
type SoftwareEditorTabKey =
  | 'softwareData'
  | 'itemLinks'
  | 'invoiceLinks'
  | 'contractLinks'
  | 'files';
type InvoiceEditorTabKey =
  | 'invoiceData'
  | 'itemLinks'
  | 'softwareLinks'
  | 'contractLinks'
  | 'files';
type ContractEditorTabKey =
  | 'contractData'
  | 'events'
  | 'itemLinks'
  | 'softwareLinks'
...
const locationFloorplanImageExtensions = new Set([
  '.jpg',
  '.jpeg',
  '.png',
  '.gif',
...
let bootstrapSnapshotPromise: Promise<Record<string, Record<string, unknown>[]> | null> | null =
  null;
...
const agentContacts = ref<AgentContactRow[]>([
...
type NonSoftwareRelationSortScope =
  ...
  | 'fileSoftware'
  | 'fileContracts';
const nonSoftwareRelationSortState = reactive<
  Record<NonSoftwareRelationSortScope, { key: string; direction: SortDirection }>
>({
...
const resourceEditorDependencies: Record<
  string,
  { relationKeys: RelationCacheKey[]; bootstrapKeys: string[] }
> = {
  items: {
...
const resourceMutationImpact: Record<
  string,
  { relationKeys: RelationCacheKey[]; bootstrapKeys: string[] }
> = {
...
const ascPrimaryResourceKeys = new Set([
  'software',
  'invoices',
  'files',
  'contracts',
...
const isEditingInvoiceFile = computed(
  () => isFileResource.value && Number(selectedId.value ?? 0) > 0 && isInvoiceTypeFile.value
);
...
function formatRackOptionLabel(item: Record<string, unknown>) {
  ...
  const units = Number.parseInt(
    String(
      item.usize ?? item.uSize ?? item.height ?? fallback?.usize ?? fallback?.uSize ?? ''
    ).trim(),
    10
  );
...
function toFieldOptionLabel(field: ResourceField, item: Record<string, unknown>) {
  if (resource.value?.key === 'contracts' && field.key === 'parentId') {
    ...
    return decodeHtmlEntities(
      [id && `${id}:`, title].filter(Boolean).join(' ') || String(item.id ?? '')
    );
  }
...
function getRackUnitSpan(startUnit: number, units: number, reverse: boolean) {
  if (!Number.isFinite(startUnit) || startUnit <= 0 || !Number.isFinite(units) || units <= 0)
    return [] as number[];
  return reverse
...
function getFieldOptions(field: ResourceField) {
  ...
  if (field.optionsKey) {
    ...
    return filtered.map(item => ({
      label: toFieldOptionLabel(field, item),
...
function upsertPendingLocationArea(areaName: string) {
  ...
  if (editingID > 0) {
    const index = locationAreas.value.findIndex(
      row => Number(row.id ?? 0) === editingID && isPendingLocationArea(row)
    );
    if (index >= 0) {
...
async function saveLocationArea() {
  ...
  error.value = '';
  const editingRow = editingLocationAreaId.value
    ? findLocationAreaRowById(editingLocationAreaId.value)
    : null;
...
function ensureAgentRows() {
  if (agentContacts.value.length === 0)
...
function applyEditorDataDirtyPayload(payload: EditorDataDirtyPayload) {
  for (const key of normalizeRelationCacheKeys(payload.relationKeys))
    dirtyRelationCacheKeys.add(key);
  for (const key of normalizeBootstrapLookupKeys(payload.bootstrapKeys))
    dirtyBootstrapLookupKeys.add(key);
}
...
function broadcastEditorDataDirty(payload: {
  relationKeys?: RelationCacheKey[];
  bootstrapKeys?: string[];
}) {
...
function broadcastResourceMutation(
  resourceKey: string,
  extra?: { relationKeys?: RelationCacheKey[]; bootstrapKeys?: string[] }
) {
...
async function fetchBootstrapSnapshot() {
  ...
  bootstrapSnapshotPromise = api
    .get('/bootstrap')
    .then(({ data }) =>
      data && typeof data === 'object' ? (data as Record<string, Record<string, unknown>[]>) : null
    )
    .catch(() => null)
    .finally(() => {
...
async function refreshDirtyEditorDependencies(resourceKey: string) {
  ...
  const [refreshedRelationKeys, bootstrapUpdated] = await Promise.all([
    relationKeys.length > 0
      ? refreshRelationCache(relationKeys)
      : Promise.resolve([] as RelationCacheKey[]),
...
const managedLinkedFileRows = computed<ManagedLinkedFileRow[]>(() => {
  ...
  const upsertRow = (
    fileID: number,
    source: FileRelationRow,
    options?: { direct?: boolean; invoiceID?: number; invoiceNumber?: string }
  ) => {
...
function canRemoveManagedLinkedFile(
  row: Pick<ManagedLinkedFileRow, 'typeID' | 'typeDesc' | 'directlyLinked'> | null | undefined
) {
  if (!canWrite.value) return false;
...
function toggleSoftwareRelationSort(
  scope: 'items' | 'invoices' | 'contracts' | 'files',
  key: string
) {
...
function getSoftwareRelationSortIcon(
  scope: 'items' | 'invoices' | 'contracts' | 'files',
  key: string
) {
  const state = softwareRelationSortState[scope];
...
function sortItemRelationRows(
  rows: ItemRelationRow[],
  selected: Set<number>,
...
const softwareContractRelationRows = computed<ContractRelationRow[]>(() => {
  const selected = new Set(getMultiValues('contractLinks').map(x => Number(x)));
  const keyword = normalizeKeyword(contractLinkFilter.value);
  const rows = buildContractRelationSource().filter(row =>
    containsKeyword(keyword, row.id, row.contractor, row.title)
  );
...
const itemItemRelationRows = computed<ItemRelationRow[]>(() => {
  const selected = new Set(getMultiValues('itemLinks').map(x => Number(x)));
  return sortItemRelationRows(
    itemRelationRowsForItemTab.value,
    selected,
    nonSoftwareRelationSortState.itemItems
  );
});

const itemInvoiceRelationRows = computed<InvoiceRelationRow[]>(() => {
  const selected = new Set(getMultiValues('invoiceLinks').map(x => Number(x)));
  return sortInvoiceRelationRows(
    invoiceRelationRowsForItemTab.value,
    selected,
    nonSoftwareRelationSortState.itemInvoices
  );
});

const itemSoftwareRelationRows = computed<SoftwareRelationRow[]>(() => {
  const selected = new Set(getMultiValues('softwareLinks').map(x => Number(x)));
  return sortSoftwareRelationRows(
    softwareRelationRowsDetailed.value,
    selected,
    nonSoftwareRelationSortState.itemSoftware
  );
});

const itemContractRelationRows = computed<ContractRelationRow[]>(() => {
  const selected = new Set(getMultiValues('contractLinks').map(x => Number(x)));
  return sortContractRelationRows(
    contractRelationRows.value,
    selected,
    nonSoftwareRelationSortState.itemContracts
  );
});
...
const invoiceItemRelationRows = computed<ItemRelationRow[]>(() => {
  const selected = new Set(getMultiValues('itemLinks').map(x => Number(x)));
  return sortItemRelationRows(
    itemRelationRowsForDnsTab.value,
    selected,
    nonSoftwareRelationSortState.invoiceItems
  );
});
...
const invoiceSoftwareRelationRows = computed<SoftwareRelationRow[]>(() => {
  ...
  return sortSoftwareRelationRows(
    softwareRelationRowsDetailed.value,
    selected,
    nonSoftwareRelationSortState.invoiceSoftware
  );
});
...
const invoiceContractRelationRows = computed<ContractRelationRow[]>(() => {
  ...
  return sortContractRelationRows(
    contractRelationRows.value,
    selected,
    nonSoftwareRelationSortState.invoiceContracts
  );
});
...
const contractItemRelationRows = computed<ItemRelationRow[]>(() => {
  ...
  return sortItemRelationRows(
    itemRelationRowsForDnsTab.value,
    selected,
    nonSoftwareRelationSortState.contractItems
  );
});
...
const contractSoftwareRelationRows = computed<SoftwareRelationRow[]>(() => {
  ...
  return sortSoftwareRelationRows(
    softwareRelationRowsDetailed.value,
    selected,
    nonSoftwareRelationSortState.contractSoftware
  );
});
...
const contractInvoiceRelationRows = computed<InvoiceRelationRow[]>(() => {
  const selected = new Set(getMultiValues('invoiceLinks').map(x => Number(x)));
  return sortInvoiceRelationRows(
    invoiceRelationRowsForContractTab.value,
    selected,
    nonSoftwareRelationSortState.contractInvoices
  );
});
...
const fileItemRelationRows = computed<ItemRelationRow[]>(() => {
  ...
  return sortItemRelationRows(
    itemRelationRowsForDnsTab.value,
    selected,
    nonSoftwareRelationSortState.fileItems
  );
});
...
const fileSoftwareRelationRows = computed<SoftwareRelationRow[]>(() => {
  ...
  return sortSoftwareRelationRows(
    softwareRelationRowsDetailed.value,
    selected,
    nonSoftwareRelationSortState.fileSoftware
  );
});
...
const fileContractRelationRows = computed<ContractRelationRow[]>(() => {
  ...
  return sortContractRelationRows(
    contractRelationRows.value,
    selected,
    nonSoftwareRelationSortState.fileContracts
  );
});

const itemOverviewRows = computed(() => {
  const ids = (fieldKey: string) =>
    getMultiValues(fieldKey)
      .map(v => Number(v))
...
const agentOverviewRows = computed(() => {
  ...
  if (activeAgentOverviewTab.value === 'invoicesVendor') {
    return toOverviewRows(
      agentRelatedVendorInvoiceRows.value,
      'invoices',
      formatOverviewInvoiceText
    );
  }
...
const locationAssociationSummary = computed(
  () => `${locationRelatedItemRows.value.length} / ${locationAssociatedRackCount.value}`
);
...
const selectedItemRackTotalUnits = computed(() =>
  parsePositiveInt(selectedItemRackLookupRow.value?.usize ?? selectedItemRackLookupRow.value?.uSize)
);

const selectedItemRackReverse = computed(
  () => Number(selectedItemRackLookupRow.value?.revnums ?? 0) === 1
);
...
function toRequestPayload() {
  ...
  if (['items', 'software', 'invoices', 'contracts'].includes(resource.value.key)) {
    payload.cleanupFileLinks = Array.from(
      new Set(pendingCleanupFileLinks.value.filter(id => Number.isFinite(id) && id > 0))
    ).sort((left, right) => naturalCompare(left, right));
  }
  return payload;
}
...
async function openEdit(row: GenericRow) {
  ...
  form.fileLinks = Array.isArray(data.fileLinks)
    ? data.fileLinks.map((id: unknown) => String(Number(id)))
    : [];
  if (resource.value.key === 'items') {
...
async function save() {
  ...
  try {
    ...
    if (resource.value.key === 'locations') {
      const floorplanFileError = getLocationFloorplanFileValidationError(
        form.file instanceof File ? form.file : null
      );
    ...
    if (resource.value.key === 'files') {
      const fileUploadError = getFileUploadValidationError(
        form.file instanceof File ? form.file : null
      );
      if (fileUploadError) {
...
function addLinkedFileSelection(id: unknown) {
  ...
  pendingCleanupFileLinks.value = pendingCleanupFileLinks.value.filter(
...
function unlinkLinkedFileSelection(id: unknown, scheduleCleanup: boolean) {
  ...
  setSelectedFileLinks(
    getMultiValues('fileLinks')
      .map(value => Number(value))
      .filter(value => value !== fileID)
  );
  if (scheduleCleanup && !pendingCleanupFileLinks.value.includes(fileID)) {
    pendingCleanupFileLinks.value = [...pendingCleanupFileLinks.value, fileID].sort((left, right) =>
...
async function uploadLinkedFile(scope: LinkedUploadScope) {
  ...
  try {
    ...
    recordDetail.value = data as GenericRow;
    form.fileLinks = Array.isArray(data.fileLinks)
      ? data.fileLinks.map((fid: unknown) => String(Number(fid)))
      : [];
    fileLinkFilter.value = '';
    await loadRows();
...
function showUploadTabTip(event: Event, text: string) {
  ...
  const shouldShowAbove =
    rect.bottom + estimatedHeight + 16 > window.innerHeight && rect.top > estimatedHeight + 16;
...
const selectableCurrentRows = computed(() =>
  sortedRows.value.filter(row => canDeleteMainRow(row as GenericRow))
);
const selectablePageRows = computed(() =>
  pagedRows.value.filter(row => canDeleteMainRow(row as GenericRow))
);
...
const allPageRowsSelected = computed(() => {
  ...
  return selectablePageRows.value.every(row =>
    selectedRowIdSet.value.has(rowKey(row as GenericRow))
  );
});
const allCurrentRowsSelected = computed(() => {
  if (selectableCurrentRows.value.length === 0) return false;
  return selectableCurrentRows.value.every(row =>
    selectedRowIdSet.value.has(rowKey(row as GenericRow))
  );
});
...
const visiblePages = computed(() => {
  ...
  const start = Math.max(
    1,
    Math.min(page.value - Math.floor(maxVisible / 2), total - maxVisible + 1)
  );
        ...
        <tbody>
          <tr v-for="row in pagedRows" :key="rowKey(row)">
            ...
            <td
              ...
              :key="col.key"
              :class="[
                getCellClass(col.key, row),
                { 'quick-tip': Boolean(getCellTitle(col.key, row)) },
              ]"
            ...
            >
              ...
              <template v-else-if="resource.key === 'software' && col.key === 'invoice'">
                <div
                  v-if="getSoftwareInvoiceDisplayEntries(row).length > 0"
                  class="software-invoice-display-list"
                >
                  <div
              ...
              <template v-else-if="resource.key === 'software' && col.key === 'installedon'">
                <div
                  v-if="parseSoftwareInstalledEntries(row).length > 0"
                  class="software-installed-list"
                >
                  <div
                    ...
                    >
                      ...
                      <span
                        :class="[
                          ...
                          getInstalledItemStatusClass(entry.id),
                          'quick-tip',
                        ]"
              ...
              <template v-else-if="resource.key === 'agents' && col.key === 'contacts'">
                <div
                  v-if="getAgentContactSummaryLines(row.contacts).length > 0"
                  class="agent-contact-summary"
                >
                  <div
              ...
              <template v-else-if="resource.key === 'racks' && col.key === 'occupation'">
                <div class="rack-occupation-cell">
                  <div class="rack-usage-track">
                    <div
                      class="rack-usage-fill"
                      :style="{ width: `${getRackOccupationInfo(row).percent}%` }"
                    />
                  </div>
                  <span class="rack-usage-text">
                    {{ getRackOccupationInfo(row).occupied }}U /
                    {{ getRackOccupationInfo(row).size || '-' }}U
                  </span>
                </div>
              </template>
              <template v-else-if="resource.key === 'locations' && col.key === 'floorplanfn'">
                ...
                >
                  ...
                  <span class="location-floorplan-table-link-text">{{
                    getLocationFloorplanDisplayName(row.floorplanfn)
                  }}</span>
                </button>
                <span v-else>-</span>
              </template>
      ...
      >
        ...
        <form
          ...
          :class="{
            ...
            'drawer-form-tabbed':
              isItemResource || isInvoiceResource || isContractResource || isFileResource,
          }"
          @submit.prevent="save"
        >
          <template v-if="isItemResource">
            <div class="item-tabs">
              <button
                ...
                @mouseenter="
                  tab.key === 'files'
                    ? showUploadTabTip($event, getUploadFileUnlinkTip())
                    : undefined
                "
                ...
                <section class="item-block">
                  ...
                  <label>
                    ...
                    <select v-model="form.locAreaId">
                      ...
                      <option
                        v-for="opt in itemLocAreaOptions"
                        :key="`locArea-${opt.value}`"
                        :value="String(opt.value)"
                      >
                        {{ opt.label }}
                      </option>
              ...
              <div class="item-bottom-grid hardware-item-bottom-grid">
                <section class="item-bottom-block">
                  ...
                  <div class="item-overview-list">
                    <div
                      v-for="entry in itemOverviewRows"
                      :key="`overview-${activeOverviewTab}-${entry.id}`"
                      class="item-overview-row"
                    >
                      <span class="mono item-overview-index">{{ entry.index }}:</span>
                      <button
                        class="item-overview-link quick-tip"
                        type="button"
                        :data-quick-tip="entry.tip"
                        @click="openOverviewEntry(entry)"
                      >
                        <span>{{ entry.text }}</span>
                      </button>
                    </div>
                ...
                <section class="item-bottom-block">
                  ...
                  <div
                    v-if="itemTagEditorOpen && selectedId && canWrite"
                    class="software-tag-editor item-tag-editor"
                  >
                    <input
                  ...
                  <div class="software-managed-file-list">
                    ...
                    >
                      <div class="software-managed-file-card-head">
                        <div class="software-managed-file-card-name">
              ...
              <div class="item-rel-panel">
                ...
                <div class="table-wrap item-rel-table-wrap">
                  <table class="item-rel-table">
                    ...
                    <tbody>
                      <tr
                        v-for="row in itemItemRelationRows"
                        :key="`item-rel-${row.id}`"
                        :class="{ 'is-linked': isMultiSelected('itemLinks', row.id) }"
                      >
                        <td>
                          <input v-model="form.itemLinks" type="checkbox" :value="String(row.id)" />
                        </td>
                        <td>
                          <button
                            type="button"
                    ...
                    <tbody>
                      ...
                      >
                        ...
                        <td class="item-invoice-desc-col">
                          <div
                            v-if="
                              row.description || parseInvoiceFileEntriesValue(row.files).length > 0
                            "
                            class="item-invoice-desc-cell"
                          >
                            <div v-if="row.description" class="item-invoice-desc-text">
                              {{ row.description }}
                            </div>
                            <div
                              v-if="parseInvoiceFileEntriesValue(row.files).length > 0"
                              class="item-invoice-file-list"
                            >
                              <button
                              ...
                              >
                                <span class="location-floorplan-table-link-icon">{{
                                  entry.id ? '↗' : '·'
                                }}</span>
                                <span class="location-floorplan-table-link-text">{{
                                  entry.text
            ...
            <section v-show="activeItemTab === 'logs'" class="item-tab-pane">
              ...
              <div v-else class="table-wrap item-log-wrap">
                <table class="item-log-table">
                  ...
                  <tbody>
                    <tr
                      v-for="row in itemActions"
                      :key="`log-${String(row.id ?? '')}-${String(row.entrydate ?? '')}`"
                    >
                      <td>{{ row.id ?? '-' }}</td>
              ...
              <div class="item-rel-panel linked-file-panel">
                ...
                <div class="table-wrap item-rel-table-wrap">
                  <table class="item-rel-table">
                    ...
                    <tbody>
                      <tr
                        v-for="row in softwareFileRelationRows"
                        :key="`item-file-rel-${row.id}`"
                        :class="{ 'is-linked': isMultiSelected('fileLinks', row.id) }"
                      >
                        <td>
                          <input
                            :checked="isMultiSelected('fileLinks', row.id)"
                            type="checkbox"
                            @change="toggleUploadFileSelection(row.id, getInputChecked($event))"
                          />
                        </td>
                        <td>
                          <button
                ...
                <div class="software-upload-grid">
                  ...
                  <div class="asset-field-row linked-upload-picker-row">
                    ...
                    <div class="location-floorplan-picker linked-upload-picker">
                      ...
                      <input
                        ref="itemUploadInput"
                        class="location-floorplan-input"
                        type="file"
                        @change="onItemUploadFileChange"
                      />
                    </div>
                  </div>
                </div>
                ...
                <section v-show="activeSoftwareTab === 'softwareData'" class="item-tab-pane">
                  <div class="asset-grid-2 software-data-grid">
                    ...
                    <section class="asset-block software-side-block">
                      ...
                      <div class="item-overview-list">
                        <div
                          v-for="entry in softwareOverviewRows"
                          :key="`soft-overview-${activeSoftwareOverviewTab}-${entry.id}`"
                          class="item-overview-row"
                        >
                          <span class="mono item-overview-index">{{ entry.index }}:</span>
                          <button
                      ...
                      <table class="item-rel-table software-item-rel-table">
                        ...
                        <tbody>
                          ...
                          >
                            ...
                            <td
                              class="quick-tip software-item-col-type"
                              :data-quick-tip="getSoftwareItemTypeTooltip(row)"
                            >
                              {{ row.itemType || '-' }}
                            </td>
                            <td>{{ row.manufacturer || '-' }}</td>
                      ...
                      <div class="software-upload-grid">
                        ...
                        <div class="asset-field-row linked-upload-picker-row">
                          ...
                          <div class="location-floorplan-picker linked-upload-picker">
                            ...
                            <span
                              :class="
                                selectedSoftwareUploadFileName
                                  ? 'location-floorplan-picker-name'
                                  : 'location-floorplan-picker-name muted-text'
                              "
                            >
                            ...
                            <input
                              ...
                              class="location-floorplan-input"
                              type="file"
                              @change="onSoftwareUploadFileChange"
                            />
                          </div>
                        </div>
                      </div>
            ...
            <section v-show="activeInvoiceTab === 'invoiceData'" class="item-tab-pane">
              <div class="asset-grid-2 software-data-grid">
                ...
                <section class="asset-block software-side-block">
                  ...
                  <div class="item-overview-list">
                    <div
                      v-for="entry in invoiceOverviewRows"
                      :key="`inv-overview-${activeInvoiceOverviewTab}-${entry.id}`"
                      class="item-overview-row"
                    >
                      <span class="mono item-overview-index">{{ entry.index }}:</span>
                      <button
              ...
              <div class="item-rel-panel">
                ...
                <div class="table-wrap item-rel-table-wrap">
                  <table class="item-rel-table">
                    ...
                    <tbody>
                      <tr
                        v-for="row in invoiceItemRelationRows"
                        :key="`inv-item-rel-${row.id}`"
                        :class="{ 'is-linked': isMultiSelected('itemLinks', row.id) }"
                      >
                        <td>
                          <input v-model="form.itemLinks" type="checkbox" :value="String(row.id)" />
              ...
              <div class="item-rel-panel linked-file-panel">
                ...
                <div class="table-wrap item-rel-table-wrap">
                  <table class="item-rel-table">
                    ...
                    <tbody>
                      <tr
                        v-for="row in invoiceFileRelationRows"
                        :key="`inv-file-rel-${row.id}`"
                        :class="{ 'is-linked': isMultiSelected('fileLinks', row.id) }"
                      >
                        <td>
                          <input
                  ...
                  <div class="asset-field-row linked-upload-picker-row">
                    ...
                    <div class="location-floorplan-picker linked-upload-picker">
                      ...
                      <input
                        ...
                        class="location-floorplan-input"
                        type="file"
                        :accept="invoiceUploadAccept"
                        @change="onInvoiceUploadFileChange"
                      />
                    </div>
                  </div>
                </div>
            ...
            <section v-show="activeContractTab === 'contractData'" class="item-tab-pane">
              <div class="asset-grid-2">
                ...
                <section class="asset-block">
                  ...
                  <div class="item-overview-list">
                    <div
                      v-for="entry in contractOverviewRows"
                      :key="`contr-overview-${activeContractOverviewTab}-${entry.id}`"
                      class="item-overview-row"
                    >
                      <span class="mono item-overview-index">{{ entry.index }}:</span>
                      <button
              ...
              <template v-else>
                <div class="table-wrap item-log-wrap">
                  <table class="item-log-table">
                    ...
                    <tbody>
                      <tr
                        v-for="row in contractEvents"
                        :key="`contract-event-${String(row.id ?? '')}`"
                      >
                        <td>{{ row.id ?? '-' }}</td>
                        <td>{{ row.siblingid || '-' }}</td>
                ...
                <form
                  v-if="canWrite"
                  class="settings-grid section-gap"
              ...
              <div class="item-rel-panel">
                ...
                <div class="table-wrap item-rel-table-wrap">
                  <table class="item-rel-table">
                    ...
                    <tbody>
                      <tr
                        v-for="row in contractItemRelationRows"
                        :key="`contr-item-rel-${row.id}`"
                        :class="{ 'is-linked': isMultiSelected('itemLinks', row.id) }"
                      >
                        <td>
                          <input v-model="form.itemLinks" type="checkbox" :value="String(row.id)" />
                ...
                <div class="software-upload-grid">
                  ...
                  <div class="asset-field-row linked-upload-picker-row">
                    ...
                    <div class="location-floorplan-picker linked-upload-picker">
                      ...
                      <span
                        :class="
                          selectedContractUploadFileName
                            ? 'location-floorplan-picker-name'
                            : 'location-floorplan-picker-name muted-text'
                        "
                      >
                      ...
                      <input
                        ...
                        class="location-floorplan-input"
                        type="file"
                        @change="onContractUploadFileChange"
                      />
                    </div>
                  </div>
                </div>
            ...
            <section v-show="activeFileTab === 'fileData'" class="item-tab-pane">
              <div class="asset-grid-2">
                ...
                <section class="asset-block">
                  ...
                  <div class="item-overview-list">
                    <div
                      v-for="entry in fileOverviewRows"
                      :key="`file-overview-${activeFileOverviewTab}-${entry.id}`"
                      class="item-overview-row"
                    >
                      <span class="mono item-overview-index">{{ entry.index }}:</span>
                      <button
              ...
              <div v-if="canEditNonInvoiceFileAssociations" class="item-rel-panel">
                ...
                <div class="table-wrap item-rel-table-wrap">
                  <table class="item-rel-table">
                    ...
                    <tbody>
                      <tr
                        v-for="row in fileItemRelationRows"
                        :key="`file-item-rel-${row.id}`"
                        :class="{ 'is-linked': isMultiSelected('itemLinks', row.id) }"
                      >
                        <td>
                          <input v-model="form.itemLinks" type="checkbox" :value="String(row.id)" />
          ...
          <template v-else-if="isAgentResource">
            <div class="asset-grid-2">
                ...
                <label>
                  ...
                  >
                    <select v-model="form.types" multiple>
                      <option
                        v-for="opt in getOptionsByFieldKey('types')"
                        :key="`agent-type-${opt.value}`"
                        :value="String(opt.value)"
                      >
                        {{ opt.label }}
                      </option>
              ...
              <section class="asset-block">
                ...
                <div class="item-overview-list">
                  <div
                    v-for="entry in agentOverviewRows"
                    :key="`agent-overview-${activeAgentOverviewTab}-${entry.id}`"
                    class="item-overview-row"
                  >
                    <span class="mono item-overview-index">{{ entry.index }}:</span>
                    <button
                  ...
                  <table class="asset-inner-table">
                    ...
                    <tbody>
                      <tr v-for="(row, idx) in agentContacts" :key="`agent-contact-${idx}`">
                        <td class="agent-contact-delete-col">
                          <button
                            class="ghost-btn small-btn"
                            type="button"
          ...
          <template v-else-if="isLocationResource">
            <div class="asset-grid-3 location-asset-grid">
                ...
                <div class="asset-field-row">
                  ...
                  <div class="location-floorplan-picker">
                    ...
                    <input
                      ...
                      class="location-floorplan-input"
                      type="file"
                      :accept="locationFloorplanAccept"
                      @change="handleLocationFloorplanChange"
                    />
                  </div>
                </div>
                <p class="muted-text">
              ...
              <section class="asset-block location-areas-block">
                ...
                <div class="table-wrap item-log-wrap">
                  <table class="item-log-table">
                    ...
                    <tbody>
                      <tr
                        v-for="row in sortedLocationAreas"
                        :key="`loc-area-${String(row.id ?? '')}`"
                      >
                        <td>{{ row.id ?? '-' }}</td>
                        <td>{{ row.areaname ?? '-' }}</td>
                        <td v-if="canWrite" class="actions-cell">
              ...
              <section class="asset-block location-overview-block">
                ...
                <div class="item-overview-list">
                  <div
                    v-for="entry in locationOverviewRows"
                    :key="`loc-overview-${activeLocationOverviewTab}-${entry.id}`"
                    class="item-overview-row"
                  >
                    <span class="mono item-overview-index">{{ entry.index }}:</span>
                    <button
                ...
                <div class="rack-occupation-row">
                  ...
                  <div class="rack-occupation-value">
                    <div
                      ...
                      :data-quick-tip="`${rackOccupationUnits}/${rackTotalUnits}U`"
                    >
                      <div class="rack-occupation-bar">
                        <div
                          class="rack-occupation-fill"
                          :style="{ width: `${rackOccupationPercent}%` }"
                        />
                      </div>
                    </div>
                  </div>
