...
- **权限控制**：完全访问 / 只读两级权限
- **操作审计**：所有写操作记录到历史日志
- **自动备份**：每日 0 点自动 VACUUM INTO 备份数据库，支持配置每日备份保留天数，Schema 变更前也会自动备份
...
| 报表 | 内置多种统计报表 |
| 数据库导入 | 支持从旧系统 .db 文件直接导入替换 |
| 自动备份 | 每日 0 点自动备份数据库，支持配置每日备份保留天数，Schema 变更前也会自动备份 |
...
| `ITDB_DAILY_BACKUP_RETENTION_DAYS` |       `0`        | 每日自动备份保留天数，`0` 表示不自动清理；模板建议 `30` |
...
可通过 `ITDB_DAILY_BACKUP_RETENTION_DAYS` 设置每日自动备份保留天数：

- 未配置或设置为 `0` 时，不自动清理历史每日备份
- 设置为正整数时，系统会在每日备份成功后清理超过保留天数的 `itdb-YYYYMMDD.db` 文件
- 清理范围仅包含每日自动备份，不会删除导入前或 Schema 变更前生成的安全备份
...
| 版本 | 发布日期 | 版本说明 | 详细日志 |
|:----:|:--------:|:--------:|:--------:|
| v1.0.0 | 2026-06-27 | 首个正式版本，完成 Go + Vue3 前后端分离重构、核心资产管理、Swagger API 文档、Docker Compose 部署、数据库迁移和自动备份保留策略 | [verchanglog/v1.0.0.md](verchanglog/v1.0.0.md) |
